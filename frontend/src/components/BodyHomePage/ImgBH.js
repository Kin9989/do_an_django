...
export default function TitlebarImageList() {
    ...
    return (
        <ImageList sx={{ width: 500, height: 322 }}>


            {/* {reversedBlogs.map(blog => (
                <div key={blog.id}>
                    <ImageListItem key="Subheader" cols={2}>
                        <ListSubheader component="div">{blog.description}</ListSubheader>
                    </ImageListItem>
                </div>
            ))} */}
            {reversedBlogs.map((blog) => (
                <ImageListItem key={blog.id}>
                    <img
                        // srcSet={`${blog.image}`}
                        src={`${blog.image}`}
                        alt={blog.title}
                    ...
                    <ImageListItemBar
                        title={blog.title}
                        subtitle={formatDate(blog.created_at)}
